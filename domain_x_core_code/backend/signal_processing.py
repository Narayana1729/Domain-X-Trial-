...
def extract_vibration_features(df, fs=1000):
    ...
    rpm_raw = float(df['rpm'].mean()) if 'rpm' in df else 1800.0
    ...
    if rpm_raw <= 10.0:
        valid_idx = np.where((freqs >= 5.0) & (freqs <= 350.0))[0]
        if len(valid_idx) > 0 and np.max(magnitudes[valid_idx]) > 0.002:
            peak_idx = valid_idx[np.argmax(magnitudes[valid_idx])]
            f_rot_est = float(freqs[peak_idx])
            rpm_mean = f_rot_est * 60.0
        else:
            rpm_mean = 0.0
    else:
        rpm_mean = rpm_raw
