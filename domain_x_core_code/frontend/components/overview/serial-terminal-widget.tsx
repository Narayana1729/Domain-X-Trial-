    ...
    <div className="rounded-xl border border-emerald-500/30 bg-neutral-950 p-5 shadow-2xl space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-neutral-800">
          ...
          <div>
            <h3 className="text-sm font-semibold text-neutral-100 flex items-center gap-2">
              ...
              {isHardwareActive ? (
                ...
                <span className="inline-flex items-center gap-1 text-[10px] uppercase font-mono px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30">
                  <span className="h-1.5 w-1.5 rounded-full bg-amber-400" />
                  Historical Dataset Stream (Offline)
                </span>
              )}
            </h3>
            <p className="text-xs text-neutral-400 font-mono">
              Format:{" "}
              <span className={isHardwareActive ? "text-emerald-400" : "text-amber-400"}>
                {isHardwareActive ? "✅ LIVE READ" : "📜 HISTORICAL DATASET READ"}: {"{ accel_x, accel_y, accel_z, rpm, temp }"}
              </span>
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs font-mono text-neutral-300">
          {onReconnectUsb && (
            <button
              onClick={onReconnectUsb}
              className="px-3 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-neutral-950 font-bold font-sans flex items-center gap-1.5 shadow-lg shadow-emerald-500/20 transition-all active:scale-95 cursor-pointer"
              title="Instantly rescan and lock onto USB serial port"
            >
      ...
      {/* Terminal Window */}
      <div ref={containerRef} className="relative rounded-lg bg-black/90 border border-neutral-800 p-4 font-mono text-xs text-emerald-400 h-48 overflow-y-auto space-y-1 shadow-inner scrollbar-thin scrollbar-thumb-neutral-800">
        {logs.length === 0 ? (
